...
(async()=>{
 ...
 try{
  ...
  const storedStudent=archive.students[0];
  storedStudent.attemptId='history-correction';storedStudent.scores.part2=25;storedStudent.scores.objectiveTotal=49;
  storedStudent.objectiveCorrection={attemptId:storedStudent.attemptId,sourceKey:require('../js/core/objective-correction.js').sourceKey(storedStudent),beforePart2:20,afterPart2:25,delta:5};
  ...
  assert.match(await student.textContent(),/소계 49점 · 서술 대기/);
  ...
  assert.match(await detail.textContent(),/소계 49점 · 서술 대기/);
  assert.match(await detail.locator('[data-objective-correction]').textContent(),/20점에서 25점으로 정정/);
  ...
  assert.equal(await page.evaluate(value=>archiveStudentScoreLabel({...value,scores:{part2:25}}),storedStudent),'단답형 25 / 30점 · 정정 +5점');
  assert.equal(await page.evaluate(value=>renderArchivedStudentAnswer({...value,submittedAt:'resubmitted'},4).includes('data-objective-correction'),storedStudent),false);
