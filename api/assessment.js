...
problem: 현재·목표 상태와 필요한 조건이 이해 가능하고 모순 없는가. logic: 카드의 순서·조건·행동이 목표 달성에 타당한가. consistency: 카드와 기호·분기의 대응이 일치하는가. flow: 연결·분기·종료·입출력이 목적에 맞는가. 노드 개수만으로 논리 정답을 판정하지 않는다. 판단(Decision) 블록의 '아니오(no)' 분기 후 처리(Process) 블록을 거쳐 다시 판단 블록으로 돌아오는 구조는 완벽하고 타당한 반복(Loopback) 구조이므로 절대 논리적 결함으로 감점하지 않는다. 해석이 모호하면 uncertainties에 적고 교사 확인을 요청한다. 같은 결함을 여러 항목에서 기계적으로 중복 감점하지 않는다.
