...
module.exports = async (req, res) => {
  ...
  try {
    if(!await reserveAiQuota(token,process.env.FIREBASE_PROJECT_ID||'donghyun-algo',process.env.AI_DAILY_LIMIT))return res.status(429).json({error:'오늘의 AI 도움 사용량에 도달했습니다. 직접 실행·수정하고 AI 검사는 사용량이 회복된 뒤 다시 시도해 주세요.'});
