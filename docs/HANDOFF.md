...
| 구분 | 인수인계 기준 |
|---|---|
| 운영 기준 | `main` · 평가 점수 조회 절감 PR #13 merge `64255cc` 운영 반영. 개인 보고서 정리와 V4 정정 PR #9의 운영 은행·DB 상태 유지 |
| 현재 작업 | `codex/part3-rubric-calibration`: 새 기준·성적표·정정·복원 구현과 검증. 실제 학생 재채점/점수 교체는 구체적 외부 목적지 동의 대기. 병합/배포 상태는 DEPLOYMENT.md 최신 기록 참조 |
| 최신 제품 수정 | 학생 점수의 주기적 반복 조회를 기존 문서 변경 알림으로 대체. 교사 전체 성적은 제출된 지필 답안이 바뀔 때 갱신. 제출 취소·재제출·검토 확정·회차 전환 유지 |
| 전달 기준 | 전체 테스트 152개·정적 검사 79개·PR 검사 성공. 로컬 Edge 회귀 24개 및 전용 조회 검사 성공, 페이지·콘솔 오류 0. Production Ready와 운영 파일 일치·ping 200·비로그인 점수 요청 401 확인. 실제 학생 저장·운영 로그인 및 실제 비용 절감률은 이번 검사에 포함하지 않음 |
