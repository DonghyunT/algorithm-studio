...
2026-09-24 승인된 후속 구현: 종료 V4 전체(현재·보관)를 비교하고 공식 명칭 원칙·기존 인정 답안은 유지하며 맥락상 같은 표현을 추가한다. 단답형 30문항 검토 후 26문항에 152개 표현을 추가했다. 종료 26회차·제출 184건 중 현재 24건이 각각 5점 상승할 예정이며 보관 변경은 0건이다. 전체 테스트 137개와 보관 화면 로컬 브라우저 검사가 통과했다. 운영 은행·학생 문서는 아직 수정하지 않았다. [PRD 3.3.1절](../PRD.md)과 [정정·복구 절차](V4_OBJECTIVE_CORRECTION.md)를 먼저 읽는다. 적용 계획과 운영 은행/원본 학생 백업은 이 PC의 Git·배포 제외 `scratch/v4-review/`에만 있고 다른 PC로 자동 전달되지 않는다. 적용 전 소스 변경 여부를 재확인하고 승인된 운영 배포와 은행 교체 후 현재 학생의 정정 이력을 기록한다. 과거 보관 원본과 Part 3를 변경하지 않는다.
