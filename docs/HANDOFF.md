...
|---|---|
| 현재 운영 브랜치 | main |
| 현재 작업 브랜치 | main (codex/student-score-hold-to-reveal 병합 완료) |
...
| 최신 제품 코드 커밋 | `559fc09` — V4 학생이 점수 보기 버튼을 누르는 동안 서버 객관·단답 참고 점수를 확인하도록 수정하여 현재 main에 병합 |
| 배포 기록 | Vercel Production (`https://algorithm-studio-ten.vercel.app/`)에 `559fc09` 반영. 운영 정적 JS·HTML HTTP 200과 새 코드 서빙, 학생 탭 콘솔 오류 없음을 확인했으며, 읽기 전용 운영 탭에는 새 V4 제출 카드가 없어 실제 버튼 누르기까지는 확인하지 못함(상세: [배포 기록](DEPLOYMENT.md)) |
| 현재 평가 UI | [순서도 흐름: ...], [알고리즘: ...] 단락이 파란 카드 섬(cbt-condition-box)으로 표시되어 들여쓰기 포함 가독성 개선. 기존 [규칙], [조건], [상황] 섬도 그대로 유지 |
| 이번 구현 완료 내용 | **formatCbtPrompt 섬 트리거 확장**: `[조건\|규칙\|상황\|조리법\|반복 규칙]` 허용 목록에 `순서도`, `알고리즘` 추가(정규식 3곳). 영향 문항 19개 모두 섬 렌더링으로 전환. 이어서 V4 객관식 답안의 0-based 저장값을 `1번 (선지 내용)`으로 표시하고, 학생 본인에게만 객관·단답 참고 점수를 누르는 동안 공개. |
