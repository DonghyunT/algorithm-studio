...
|---|---|
| 운영 기준 | `main` · `5ada837` |
| 작업 브랜치 | `codex/excel-export-and-student-review` (`c2358c4`) |
| 마지막 제품 수정 | 멀티 시트 엑셀 성적표(학급종합 + 학생별 A4 출력 리포트 + 나이스 양식) 다운로드, 서버 점수 일괄 채점 연동(`class-grades`), 1차 채점 명칭 일원화, 학생용 문항별 답안/정답 확인 모달(`student-review`), 학생 제출 화면 Part 3 1차 채점 실시간 반영 |
...
최근 완료한 제품 변경은 성적표 멀티 시트 엑셀 개편 및 학생용 답안/정답 확인 모달 구축(`c2358c4`)입니다. 외부 라이브러리 없이 순수 Vanilla JS OpenXML ZIP(`.xlsx`, `js/core/excel-export.js`)으로 단일 파일 내 `학급종합`, 학생수만큼의 A4 1장 맞춤형 개별 성적표(`01_강수아`), 나이스 입력 시스템 복사/붙여넣기 전용 `나이스(NEIS) 양식` 시트를 생성합니다. 학급명 `2-6`이 엑셀에서 날짜(`02월 06일`)로 왜곡되는 현상을 `inlineStr`로 방지하였으며, V4 실전평가에서 '서버 채점 확인' 텍스트 대신 실제 채점 점수가 반영되도록 `class-grades` 일괄 채점 API를 연동했습니다. 학생 화면에는 교사 관제탑의 AI 분석 완료 시 Part 3 1차 채점 점수가 실시간 반영되며, 시험 종료 후 학생이 왜 이 점수인지 납득할 수 있는 문항별 답안/정답 확인 모달(시험 중 치팅 방지 락 적용)을 제공합니다. (단위 테스트 100/100 통과, tools/check.cjs 68개 스크립트 정적 검사 통과)

## 2. 읽는 순서와 다음 작업

1. [AGENTS](../AGENTS.md) → [INTENT](../INTENT.md) → [PRD](../PRD.md) → 이 문서를 읽습니다.
2. 작업 브랜치 `codex/excel-export-and-student-review` 검토 및 main 병합 준비.
...
> AGENTS.md, INTENT.md, PRD.md, docs/HANDOFF.md를 읽고 이어가 주세요. 운영 전달 기준은 main이며 문서 정비 브랜치는 codex/docs-current-state입니다. 2026-09-19 푸시·병합·배포 승인이 있었으므로 Git과 PR 기록에서 완료 여부를 확인해 주세요. Git 상태와 원격 반영 여부를 먼저 확인하고 로컬 변경을 보존해 주세요. V4는 운영 중이고 V3는 모의평가·기존 회차 호환용입니다. 완료한 제품을 재배포하지 말고 문서 검토와 남은 화면 점검·회차 재현·보관 복구 과제를 구분해 주세요. 운영 자료 변경과 main 병합은 해당 승인을 확인한 뒤 진행합니다.
