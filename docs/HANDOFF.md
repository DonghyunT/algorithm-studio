...
| 마지막 제품 수정 | 1) 컴퓨터실 시계 오차(Clock Skew) 원천 방어, 2) 학생 관리 모달 데스크탑 좌우 2단 분할 개편, 3) 제출 취소 및 풀던 답안 100% 보존 복귀 (`reopenStudentExam`), 4) 학급 전체/개별 시간 연장, 5) 제출 완료 화면 점수 섬 통합 및 하단 버튼 재배치, 6) 실시간 리스너 영구 지속성(`attachAssessmentListeners`), 7) 개별 시간 연장 즉시 반영 및 타이머 연속성 부활, 8) 강제 정상 제출 마감 실시간 화면 전환, 9) 진행 중 평가 입장 시 즉시 시험 화면 직행 |
| 전달 기준 | 단위 테스트 123개 100% 통과, 정적 구문 검사 72개 OK, 로컬 라이브 검증 완료 |
...
2. 현재 모든 핵심 제품 변경 사항은 `main`에 병합 및 운영 배포 완료되었습니다.
...
> AGENTS.md, INTENT.md, PRD.md, docs/HANDOFF.md를 읽고 이어가 주세요. 운영 전달 기준은 main(`311ea4c`)이며 멀티 디바이스 동기화 및 문서 최신화 작업 브랜치는 docs/sync-multi-device-context입니다. 2026-09-21 기준 1) 결시생 개별 30분 추가 응시 복원 및 기기 무관 로그인(`ownerUid: null`), 2) 멀티 시트 엑셀 성적표(A4 맞춤 인쇄, Part 3 서식·루브릭·1차 채점 합산 고도화), 3) 교사 관제탑 좌석 바둑판 실시간 서버 성적 연동, 4) 시험 종료 후 입장 원천 차단 및 좌석 보호까지 모두 main 병합 및 Vercel 운영 배포가 완료되었습니다(단위 테스트 117개 통과, tools/check.cjs 71개 스크립트 OK). 다른 PC로 이동 시 최신 main을 pull 받고 작업 목적에 맞는 브랜치를 생성하여 진행해 주세요. AGENTS.md는 최고 규범(헌법)이므로 명시적 사전 승인 없이 수정하지 않습니다.
