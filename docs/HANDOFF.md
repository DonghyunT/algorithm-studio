...
| 인수인계 전달 기준 | 최신 main에 CBT 3·4차 정밀 고도화(용어 통일, 좌우 섬 높이 100% 일치, 17-2 캔버스 언클램프·720px 확장 및 상단 밀착, 단답형 자동 포커스 & Enter 키) 통합 및 프로덕션 배포 완료 |
| 최신 제품 코드 | `cbt-question-grid`, 슬림 레일(56px), '현재 상태' 용어 표준화, 17-1 카드 상단 1px 정렬, 좌우 섬 높이 100% 일치, 17-2 캔버스 높이 고정 해제(`cbt-step2-unclamped`) 및 720px 확장, 상단 들뜸 0px 제거, 단답형 자동 포커스 & Enter 다음 문항 이동, `active:scale-96` 스프링 모션 |
...
| 이번 구현 완료 내용 | 1) **교육과정 표준 용어 통일**: '시작 상태'를 2022 개정 교육과정 표준인 '현재 상태'로 전면 일원화<br>2) **좌우 섬 높이 100% 일치 (Pixel-Perfect Alignment)**: 사이드바와 문제 섬의 데스크탑 높이 규칙(`height: calc(100vh - 150px) !important; max-height: 860px !important;`)을 일원화하여 상·하단 1px 오차 없이 일치<br>3) **순서도 조립(17-2) 섬 높이 고정 해제 & 상단 들뜸 0px 제거**: 17-2단계 860px 높이 제한 해제(`cbt-step2-unclamped`), 캔버스 720px 확장 및 상단 12px 마진 제거로 기획서/툴박스 온전 노출 및 밀착 마감<br>4) **단답형(11~16번) 자동 포커스 & Enter 키 지원**: 문항 진입 시 정답 입력창에 즉시 자동 포커스 및 커서 배치, Enter 키 입력 시 다음 문항으로 쾌속 이동<br>5) **모든 문항 조건 카드형 표준화 & 토스 스타일 모션**: `.cbt-condition-box` 스타일링 및 `active:scale-96` 햅틱 피드백 적용<br>6) **77개 전체 단위 테스트 100% 통과** (통과율 100%) |
