...
| 구분 | 인수인계 기준 |
|---|---|
| 운영 기준 | `main` · 개인 보고서·임시 파일 정리 PR #11 merge `b752b6b` 운영 반영. V4 정정 PR #9의 운영 은행·DB 상태 유지 |
| 현재 작업 | `codex/reduce-evaluation-reads`: 학생 점수 반복 조회와 교사 성적 전체 재조회 절감. 로컬 구현·검증 후 승인된 운영 반영 진행 중. 완료 여부는 배포 기록 참조 |
| 최신 제품 수정 | 문항별 인정 답안 확대, 원본 보존·중복 방지·복구 도구, 향후 보관된 정정 답안의 변경 전후 표시. 기존 보관 원본과 Part 3 유지 |
| 전달 기준 | 전체 테스트 137개·정적 검사·PR 검사 성공. Production Ready, 운영 은행 정확 일치·정적 파일 일치·DB 정정/원본 보존 재조회·담당 범위 접근 확인. 실제 CDN 합성 표시 1440/390px에서 오류 0. 세부 제한은 배포 기록 참조 |
