...
| 이번 승인 범위 | V3 모의 → V4 실제 평가 방향과 AI 채점 토의 기록·인수인계. 구현·운영 변경은 미승인 |
| 이번 토의 문서 작업 | codex/evaluation-discussion-handoff. main 병합 전에는 이 브랜치의 원격/PR 상태를 확인 |
...
> AGENTS.md, INTENT.md, PRD.md, docs/HANDOFF.md를 읽고 작업을 이어가 주세요. 먼저 기존 작업을 보존하고 최신 main을 받은 뒤 새 작업 브랜치를 만들어 주세요. V3은 현재 운영이고 V4는 기반만 배포된 상태입니다. 최신 토의 문서 브랜치의 main 반영 여부도 확인해 주세요. 이번 주 V3은 모의, 다음 주 V4는 실제 평가 목표이며 나머지 판단은 선생님 답변 대기입니다. PRD 3.5절과 V4 계획 12절을 먼저 읽고 임의로 구현을 시작하지 말아 주세요. 완료된 기반을 재구축하거나 재배포하지 말고 V4 계획 8~11절의 회차 재현·비공개 보관 설계와 다음 문항 품질 작업 범위를 먼저 확인해 주세요.
