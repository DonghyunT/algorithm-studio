...
| 구분 | 인수인계 기준 |
|---|---|
| 운영 기준 | `main` · CBT 순서도 조립 UI 개선, 평가 결과 화면 0점 오류 수정 및 학생 리뷰 모달 UX 개편 완료 |
| 현재 작업 | CBT 순서도 조립(Part 3 Step 2) 작업 공간 640px 고정·자연어 기획서 독립 스크롤 분리·문항 목록 섬 높이 통일, 제출 완료 화면 점수 표시 정합성 복구, 리뷰 모달 데스크톱 와이드스크린 UX 최적화 완료 및 main 병합·배포 |
| 최신 제품 수정 | 1. CBT 순서도 조립 화면(자연어 기획서 스크롤, 순서도 640px 캔버스, 문항 팔레트 높이 고정)<br>2. 평가 결과 화면 채점 소계(60점 만점) 정합성 복구 및 모의평가(V3) 정답 매칭 개선<br>3. 학생 리뷰 모달 가독성·반응형 레이아웃(max-w-4xl, 성적 요약 배너, 문항 비교 카드) 전면 개선 |
| 전달 기준 | 전체 테스트 170개(100%) 통과, 정적 검사 92개 스크립트 통과. 로컬 시연(HTTP 200) 및 데모 모드 학생/모의평가/실전평가 흐름 검증 완료 |
