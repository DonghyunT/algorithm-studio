...
| 구분 | 인수인계 기준 |
|---|---|
| 운영 기준 | `main` · Part 3 개편 PR #15 merge `fe13b21` 운영 배포 완료. 승인된 기존 회차 재채점·잠정점수 반영 완료, 학생 원본 답안·은행 유지 |
| 현재 작업 | 실제 학생 재채점과 서버 재조회 검증 완료. 감점 지침 보완과 완료 기록은 `codex/part3-grading-completion`에서 검증. 최종 병합·배포는 DEPLOYMENT.md 참조 |
| 최신 제품 수정 | 실전평가 새 여섯 항목·최저 6점·잠정/교사 정정·성적표 환산·보관 회차 출력 및 복구 도구. 기존 회차는 기준 버전 유지 |
| 전달 기준 | 전체 테스트 169개·정적 검사 92개 통과. 합성 Solar 총 40회, 마지막 정상 반복·짧은 표현/맞춤법·기본 문구·반전 구분 4개 통과. 실제 재채점·서버 백업·원본 불변 재조회 및 담당 교사 운영 화면·성적표 다운로드 확인 |
