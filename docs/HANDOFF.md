...
| 구분 | 인수인계 기준 |
|---|---|
| 운영 기준 | `main` (보관 회차 조회 PR #7 merge `992e008`; 점수 회차 격리 PR #5 merge `0c0c76e`) |
| 현재 작업 | `codex/v4-short-answer-review`: 제품 커밋 `177374c` 원격 푸시, 초안 PR #9. 인정 답안 확대·정정 도구·보관 정정 표시 구현 및 검증 완료. 운영 병합·배포 승인 대기. 은행 교체·DB 정정 미실행 |
| 최신 제품 수정 | 교사용 `이전 평가 기록` 탭에서 보관 회차와 당시 저장 답안·점수를 읽기 전용 조회. 저장 점수 재계산·보관 자료 수정은 하지 않음 |
| 전달 기준 | PR #7의 테스트 130개와 정적 검사 73개, 로컬 데모 브라우저의 1440/1024/768/390px 확인 통과. GitHub PR 검사와 merge SHA의 Vercel 체크 성공. 운영 교사 로그인·Firestore 자료 접근은 이번에 확인하지 않음 |
...
최신 재검토: 교사가 제외한 추가 표현과 그 동일 의미 변형을 비공개 수정본에서 제거하고 정정 계획·학생 확인 목록을 재생성했다. 이전 계획이나 그 해시로 실행하지 않는다. 최신 계획 식별자와 영향 규모는 비공개 자료에서 확인한다. 운영 적용 승인은 아직 대기 중이다.

2026-09-24 승인된 후속 구현: 종료 V4 전체(현재·보관)를 비교하고 공식 명칭 원칙·기존 인정 답안은 유지하며 맥락상 같은 표현을 추가한다. 교사 재검토에 따른 제외 조건도 반영했다. 전체 테스트 137개와 보관 화면 로컬 브라우저 검사가 통과했다. 운영 은행·학생 문서는 아직 수정하지 않았다. [PRD 3.3.1절](../PRD.md)과 [정정·복구 절차](V4_OBJECTIVE_CORRECTION.md)를 먼저 읽는다. 실제 인정 목록·학생별 영향·정정 규모·계획 식별자와 운영 은행/원본 학생 백업은 이 PC의 Git·배포 제외 `scratch/v4-review/`에서만 관리하며 다른 PC로 자동 전달되지 않는다. 적용 전 소스 변경 여부를 재확인하고 승인된 운영 배포와 은행 교체 후 대상 학생의 정정 이력을 기록한다. 과거 보관 원본과 Part 3를 변경하지 않는다.
...
5. 교사용 `이전 평가 기록` 조회는 PR #7, main merge `992e008`로 반영됐고 병합 커밋의 Vercel 체크가 성공했습니다. 저장된 점수를 재채점하지 않으며 V4 문항 원문이 보관되지 않은 경우 최신 문항을 대신 표시하지 않습니다. 운영 로그인·Firestore 접근 확인은 후속 검증 대상입니다. 상세 범위는 [PRD 3.4절](../PRD.md), 배포 근거는 [배포 기록](DEPLOYMENT.md)을 따릅니다.
