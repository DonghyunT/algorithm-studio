...
갱신: 2026-09-25 KST. 기능은 [PRD](../PRD.md), 운영 반영 근거와 검증 범위는 [배포 기록](DEPLOYMENT.md)이 기준입니다.

## 1. 현재 상태

| 구분 | 인수인계 기준 |
|---|---|
| 운영 코드 기준 | `main`의 제품 커밋 `6acb25141cb177cf26496318d4ebbfa03cf16584` |
| 운영 배포 | Vercel Production 배포 `dpl_DV2vPntg98HGmw59M5y4i877UYwE` · `READY` · 기본 운영 주소 연결 |
| 최신 제품 변경 | CBT 17-2 캔버스 640px·기획서 독립 스크롤, 제출 결과 지필 소계 수정, 학생 리뷰 모달 와이드스크린 개선 |
| 기록된 로컬 검증 | 단위 테스트 170개, 정적 검사 92개 스크립트, 미리보기 HTTP 200. 데모 학생·V3 모의·V4 평가 흐름은 직전 인수인계에 확인 완료로 기록 |
| 운영 UI 검증 범위 | 배포 상태와 도메인 연결은 확인. 변경 화면의 운영 브라우저 시각 검증은 확인되지 않음 |
...
위 검사 수치는 제품 커밋의 배포 기록이며 이번 문서 동기화에서 다시 실행하지 않았습니다. Vercel `READY`는 해당 커밋의 Production 배포와 별칭 연결을 뜻하며, 운영 브라우저에서 화면·콘솔을 직접 확인한 근거와는 구분합니다. 자세한 식별자와 확인 범위는 [배포 기록](DEPLOYMENT.md)을 따릅니다.

최근 완료한 제품 변경 내역 (2026-09-25):
1. **CBT Part 3 순서도 조립(Step 2) 작업 공간 및 레이아웃 최적화 (`index.html`)**:
   - 자연어 기획서 분량이 많을 때 전체 페이지가 아래로 무한정 늘어나 브라우저 배율을 50%로 줄여야 하던 문제 해결.
   - 좌측 자연어 기획서 패널에 `max-h-full overflow-y-auto` 독립 스크롤을 적용하여 긴 기획서도 자체 스크롤 가능하게 분리.
   - 우측 순서도 조립 캔버스를 쾌적한 640px 고정 높이(`h-[640px]`) 및 배경 드래그(Pan) 이동으로 최적화하여 조립 편의성 대폭 개선.
   - 좌측 문항 목록(팔레트 섬 `#eval-palette-island`)에 `self-start`를 적용하여 순서도 조립 단계에서도 다른 문항(Step 1, 객관/단답)과 동일한 균일 높이를 유지하도록 수정.
2. **평가 제출 완료 화면 점수 표시 정합성 복구 (`index.html`, `js/labs/lab-eval.js`, `js/core/flow-validation.js`)**:
   - 시험 제출 완료 화면(`#eval-screen-result`)에서 실제 점수가 6점임에도 `0점`으로 표기되던 버그 수정.
   - `index.html` 내 초기 하드코딩 `0점`을 `-- / 60점`으로 변경하여 계산 전 0점 플래시 방지.
   - 대기실 복귀(`enterWaitingRoom`) 및 로비 재진입(`openLobby`) 시 `status === 'submitted'` 상태인 학생에 대해 `calculateScores()`, `renderResult()`를 보장하여 정확한 지필 소계(`${objTotal}점 / 60점`) 주입.
   - `gradeEvaluation` 채점 시 문자열/숫자 타입 불일치 방지(`Number(ans) === Number(q.correctAnswer)`) 및 Part 2 단답형 인정 정답 목록 폴백 보완.
3. **문항별 리뷰 모달 데스크톱 와이드스크린 UX 및 디자인 전면 개편 (`index.html`, `js/labs/lab-eval.js`)**:
   - 기존 `max-w-3xl`, `90vh` 설정으로 인해 16:9 와이드 모니터에서 어색하게 좁고 높던 수직 타워 형태를 시원하고 안정적인 `max-w-4xl`, `max-h-[84vh]` 비율로 개선.
   - 모달 상단에 총 문항 수, 정답 문항 수, 정답률(%)이 포함된 '수행평가 채점 결과 요약' 그라디언트 배너 신설.
   - 문제 지문 가독성 강화(`text-sm font-bold text-slate-900`), 정답/오답 뱃지 및 '내가 선택한 답' vs '실제 정답' 비교 카드 디자인 세련화.
   - 상단 닫기(X) 버튼 및 하단 생기부 반영 안내 푸터 배치.
...
2026-09-24 Part 3 개편: [PRD 3.2절](../PRD.md#32-수행평가), [검증·복구 기록](PART3_RUBRIC_CALIBRATION.md)을 확인한다. 사용자가 Upstage Solar·사용자 소유 비공개 Preview 처리 경로를 명시 승인한 뒤 대상의 재채점·원본 감사 보존·잠정점수 교체·서버 재조회를 완료했다. 앱 재시작 후 저장된 결과를 재사용했고, 미완료 요청만 재개했다. 감점 근거 대조에서 발견한 기준 위반·누락은 추가 AI 호출 없이 바로잡아 원래 응답과 조정 내역을 비공개로 보존했다. **완료된 재채점을 다시 실행하지 않는다.** 복구 전에는 이후 교사 정정과 답안 변경을 확인한다. 보관 회차 중 지필 점수가 저장되지 않은 자료는 서술 점수만 확정 가능하며 총점·환산은 대기로 둔다.
...
> AGENTS.md, INTENT.md, PRD.md, docs/HANDOFF.md를 읽고 이어가 주세요. 운영 기준은 `main`입니다. 2026-09-25 CBT 순서도 조립 UI 개선(자연어 기획서 독립 스크롤, 640px 조립 캔버스, 문항 팔레트 높이 통일), 평가 제출 결과 화면 지필 소계 0점 표시 오류 수정, 학생용 문항 리뷰 모달 와이드스크린 UX 최적화(max-w-4xl, 정답률 요약 배너, 문항 비교 카드 개선)가 반영되었습니다. 전체 단위 테스트 170개(100%), 정적 검사 92개 스크립트가 통과되었습니다. 자세한 배포 근거는 [배포 기록](DEPLOYMENT.md)을 따릅니다. AGENTS.md는 명시적 사전 승인 없이 수정하지 않습니다.
