...
|---|---|
| 현재 운영 브랜치 | main |
| 현재 작업 브랜치 | main (codex/v4-teacher-answer-display 병합 완료) |
| 현재 평가 운영 상태 | 선생님 확인에 따라 V4 실전평가도 운영에 적용 중이며, V3는 모의평가·기존 회차 호환용으로 유지 |
| 인수인계 전달 기준 | 1) [순서도 흐름]/[알고리즘] 단락이 cbt-condition-box 섬으로 렌더링되도록 formatCbtPrompt 정규식 확장, 2) V4 교사 상세 조회의 학급 ID 전달 오류 수정, 3) V4 객관식 답안의 선지 번호·문구 표시 보완, 4) 관련 권한·평가 API 14개 테스트 및 운영 정적 파일·화면 확인 |
| 최신 제품 코드 | `js/core/classroom.js`(V4 교사 성적·문항 조회에 학급 ID 전달 및 0-based 객관식 답안을 선지 번호·문구로 표시), `js/labs/lab-eval.js`(조건 카드 섬 트리거 확장), `css/common.css`(조건 카드 들여쓰기 보존) |
| 최신 제품 코드 커밋 | `a733dc2` — V4 교사 상세 답안에 객관식 선지 번호와 내용을 표시하도록 수정하여 현재 main에 병합 |
| 배포 기록 | Vercel Production (`https://algorithm-studio-ten.vercel.app/`)에 `a733dc2` 반영. 운영 정적 JS HTTP 200과 표시 매핑을 확인했으며, 현재 읽기 전용 교사 탭은 모의평가 회차라 새 V4 제출 카드의 실제 표시까지는 확인하지 못함(상세: [배포 기록](DEPLOYMENT.md)) |
| 현재 평가 UI | [순서도 흐름: ...], [알고리즘: ...] 단락이 파란 카드 섬(cbt-condition-box)으로 표시되어 들여쓰기 포함 가독성 개선. 기존 [규칙], [조건], [상황] 섬도 그대로 유지 |
| 이번 구현 완료 내용 | **formatCbtPrompt 섬 트리거 확장**: `[조건\|규칙\|상황\|조리법\|반복 규칙]` 허용 목록에 `순서도`, `알고리즘` 추가(정규식 3곳). 영향 문항 19개 모두 섬 렌더링으로 전환. 이어서 V4 객관식 답안의 0-based 저장값을 `1번 (선지 내용)`으로 표시. |
...
2. **실전평가 V4 운영 적용·교사 상세 조회·객관식 표시 수정 완료:** Vercel Production에 V4 운영 설정·코드와 `a733dc2` 수정이 반영되어 사용 중입니다. 문법 검사 66개 스크립트와 관련 권한·평가 API 테스트 14개를 통과했고, 샘플 V4 답안으로 `학생 선택: 1번 (선지 내용) / 정답: 2번 (선지 내용)` 표시를 확인했습니다. 현재 운영 교사 탭은 모의평가 회차였으므로 새 V4 제출 카드의 실제 시각 표시 확인은 다음 V4 제출 자료에서 이어갑니다. 관련 최신 기록은 선생님 운영 확인과 [배포 기록](DEPLOYMENT.md)을 기준으로 합니다.
3. **다음 작업:**
