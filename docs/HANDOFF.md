...
| 현재 작업 | `codex/docs-consistency-cleanup` (변경 커밋 있음, 로컬 전용·원격 미게시) |
| 최신 제품 수정 | `9782b7d`: 진행 중인 평가에 늦게 입장한 학생을 시험 화면으로 전환. 앞선 2026-09-22 제품 변경은 아래 기록과 [배포 기록](DEPLOYMENT.md) 참고 |
| 전달 기준 | 2026-09-22 배포 기록: 단위 테스트 123개, 정적 검사 72개 및 프로덕션 ping 확인. 이번 문서 정리에서는 제품·운영을 재검증하지 않음 |
...
> AGENTS.md, INTENT.md, PRD.md, docs/HANDOFF.md를 읽고 이어가 주세요. 운영 기준은 `main`이며 최신 제품 코드 커밋은 `9782b7d`입니다. 현재 문서 불일치 정리 브랜치는 `codex/docs-consistency-cleanup`이고 원격에 게시되지 않았으므로, 이 변경을 다른 PC로 가져가려면 먼저 원격에 게시하고 검토·통합해야 합니다. 2026-09-22 제품 변경의 검증·배포 기록은 `docs/DEPLOYMENT.md`를 확인해 주세요. 회차별 문항·채점 재현과 보관·복구 설계는 `docs/EVALUATION_V4_PLAN.md`의 남은 과제입니다. AGENTS.md는 명시적 사전 승인 없이 수정하지 않습니다.
