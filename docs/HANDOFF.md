...
| 인수인계 전달 기준 | 최신 main에 CBT 5차 정밀 고도화(Plan A: 17-2 요약 아코디언 기본 열림, 일상생활 AI 채점 기준 캘리브레이션, 조건 추천 일상 맥락 반영, 유연한 단말 검증 및 구체적 오류 안내) 통합 및 프로덕션 배포 완료 |
| 최신 제품 코드 | `cbt-question-grid`, 슬림 레일(56px), '현재 상태' 용어 표준화, 17-2 1단계 계획 요약 기본 펼침(`open`), 중2 일상 고민 AI 채점(9~10점) 보정, 조건 추천 프롬프트 일상 맥락화, 유연한 단말 검증(`/^(?:시작|출발)/`, `/^(?:종료|끝|마침|도착)/`), 구체적 단절 블록 안내, 17-2 언클램프(720px), 단답형 자동 포커스 & Enter 다음 문항 이동 |
| 배포 기록 | Vercel Production 배포 완료 (`https://algorithm-studio-ten.vercel.app/`), 라이브 에셋 서빙 실측 검증 완료 |
| 현재 평가 UI | 단일 문항 집중 모드가 기본값 (`isCbtMode = true`), 우측 상단 '모아서 보기 / 한 문제씩 보기' 토글 지원 |
| 이번 구현 완료 내용 | 1) **17-2 순서도 조립 1단계 계획 요약 기본 열림 (`<details open>`)**: 2단계 진입 시 1단계에서 작성한 현재·목표 상태와 규칙을 잊지 않고 보면서 조립할 수 있도록 기본 펼침 적용<br>2) **중2 일상생활 고민 맞춤 AI 채점 기준 캘리브레이션 (`api/assessment.js`)**: 중2 학생들의 일상적인 단순 문제('졸린 상태' ➔ '꿀잠 자는 상태' 등)라도 논리와 순서도가 타당하면 9~10점 만점을 부여하도록 프롬프트 채점 기준 완화 및 명문화<br>3) **조건 아이디어 추천 일상 맥락 개편 (`api/assessment.js`)**: 기계 센서 대신 수면, 식사, 등교, 공부, 스마트폰 사용 시간 등 중2 학생들의 실제 일상 고민에 맞는 현실적 조건 후보를 추천하도록 프롬프트 조정<br>4) **순서도 단말 기호 유연 검증 & 구체적 오류 안내 (`js/core/flow-validation.js`)**: 단말 기호 명칭('시작!', '출발', '종료하기', '끝!', '마침', '도착' 등)을 유연하게 인정하고, 선이 끊겼을 때 구체적인 블록 이름과 판단 기호의 분기 누락 사실을 친절하게 안내<br>5) **79개 전체 단위 테스트 100% 통과** (통과율 100%) |
