...
|---|---|
| 현재 운영 브랜치 | main |
| 현재 작업 브랜치 | codex/cbt-ui-card-and-motion (CBT 4차 UI/UX 정밀 고도화 완료, 77개 단위 테스트 통과, 검토 및 메인 병합 대기) |
| 인수인계 전달 기준 | 최신 main(b29f85f) 기반 작업 브랜치에 CBT 4차 정밀 고도화(좌우 섬 높이 100% 일치, 17-2 캔버스 높이고정 해제·확장, 단답형 자동 포커스 & Enter 키 지원) 구현 및 단위 테스트 완료 |
| 최신 제품 코드 | `cbt-question-grid`, 슬림 레일(56px), '현재 상태' 용어 표준화, 17-1 카드 상단 1px 정렬, 좌우 섬 높이 100% 일치, 17-2 캔버스 높이 고정 해제(`cbt-step2-unclamped`) 및 720px 확장, 단답형 자동 포커스 & Enter 다음 문항 이동, `active:scale-96` 스프링 모션 |
| 배포 기록 | Vercel Production 배포 대기 (선생님 승인 후 `main` 병합 시 자동 배포 예정) |
| 현재 평가 UI | CBT 단일 문항 모드가 기본값 (`isCbtMode = true`), 우측 상단 '클래식 탭 모드' 토글 지원 |
| 이번 구현 완료 내용 | 1) **좌우 섬 높이 100% 일치 (Pixel-Perfect Alignment)**: 문항목록 사이드바와 문제 섬의 데스크탑 높이 규칙(`height: calc(100vh - 150px) !important; max-height: 860px !important;`)을 일원화하고 하단 여백 충돌을 교정하여 상단부터 하단까지 1px 오차 없이 일치<br>2) **순서도 조립(17-2) 섬 높이 고정 해제 & 720px 확장**: 17-2단계 진입 시 860px 높이 제한 및 overflow hidden을 해제(`cbt-step2-unclamped`)하고 캔버스를 720px로 확장하여 자연어 기획서 폼과 기호 보관함이 밑으로 잘리지 않도록 개선<br>3) **단답형(11~16번) 자동 포커스 & Enter 키 지원**: 문항 진입 시 정답 입력창에 즉시 자동 포커스 및 커서 배치, Enter 키 입력 시 다음 문항으로 쾌속 이동<br>4) **77개 전체 단위 테스트 100% 통과** (2개 신규 테스트 추가) |
