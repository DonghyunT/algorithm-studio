...
2. **1, 2단계 완료 상태:** 80문항 비공개 문제은행 생성, 도메인 균형 배정 엔진 고도화, 모의평가 AI 채점 차단 및 실전평가 전용 활성화, 한글 용어 표준화 및 단위 테스트 전수 통과 완료.
3. **다음 작업(3단계):**
   - Vercel Production에 비밀 환경 변수 등록: `EVAL_BANK_V4_JSON` (80문항 JSON 데이터), `EVAL_ASSIGNMENT_SECRET` (32자 이상 암호화 시크릿).
   - 선생님 승인 후 `codex/eval-v4-setup` 브랜치를 `main`으로 병합 및 배포.
   - 모의 학생 계정으로 로그인하여 실전평가 회차 개설 및 실제 문항 배정·풀이·제출·채점 실화면 엔드투엔드(E2E) 점검.
