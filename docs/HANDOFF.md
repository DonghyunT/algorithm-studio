...
* **현재 P0 작업(2026-09-15):** 운영 브랜치 `codex/secure-assessment-integrity`에서 V3 공개 문제은행은 보존하고, V4만 비공개 서버 문항 은행에서 학생별 문항을 전달·채점하도록 구현했습니다. 학생은 정답·유사 정답·배정 비밀값을 받지 않고, 제출 후 담당 교사만 서버 채점 결과와 문항·정답·출제 의도를 볼 수 있습니다. Firestore 기본 DB 규칙은 2026-09-15 실제 게시했습니다. V4에는 아직 실제 문항·운영 환경 변수·회차 선택 화면을 넣지 않아 V3 운영에 영향이 없습니다. 자세한 상태와 다음 단계는 [V4 계획](EVALUATION_V4_PLAN.md) 및 [V4 운영 설정](EVALUATION_V4_SETUP.md)을 먼저 확인합니다. 로컬 검사 `node tools/check.cjs`, `node --test tests/*.test.cjs`는 2026-09-15에 통과했습니다.
