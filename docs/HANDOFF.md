...
|---|---|
| 현재 운영 브랜치 | main |
| 현재 작업 브랜치 | codex/cbt-ui-card-and-motion (CBT 3차 UI/UX 정밀 고도화 완료, 75개 단위 테스트 통과, 검토 및 메인 병합 대기) |
| 인수인계 전달 기준 | 최신 main(b29f85f) 기반 작업 브랜치에 CBT 3차 정밀 고도화(용어 통일, 카드 정렬, 조건 카드 표준화, 캔버스 스크롤/확장, 잔여 그림자 제거, 스프링 마이크로 인터랙션) 구현 및 단위 테스트 완료 |
| 최신 제품 코드 | `cbt-question-grid`, 슬림 레일(56px), '현재 상태' 용어 표준화, 17-1 카드 상단 1px 정렬, 17-2 캔버스 640px+ 확장 및 동적 스크롤, `.cbt-condition-box` 스타일 및 자동 파싱, `active:scale-96` 스프링 모션 |
| 배포 기록 | Vercel Production 배포 대기 (선생님 승인 후 `main` 병합 시 자동 배포 예정) |
| 현재 평가 UI | CBT 단일 문항 모드가 기본값 (`isCbtMode = true`), 우측 상단 '클래식 탭 모드' 토글 지원 |
| 이번 구현 완료 내용 | 1) **교육과정 표준 용어 통일**: '시작 상태'를 2022 개정 교육과정 성취기준 표준인 '현재 상태'로 전면 교체 (17-1 본문, 가이드, 채점 기준, 17-2 요약 바)<br>2) **17-1 카드 상단 광학 정렬 (Optical Alignment)**: 마진 상속 버그 교정으로 좌우 두 카드의 상단 시작 높이 1px 오차 없이 완벽 수평 정렬<br>3) **전체 문항 조건 카드형 표준화 (`.cbt-condition-box`)**: 7, 10, 15번 등 조건/규칙 제시 문항을 정갈한 독립 카드 박스로 분리 표시, 단일 개행 파싱 보강 및 불필요한 대괄호 태그 자동 정제<br>4) **15번 등 조건 카드 시각 계층/여백 최적화**: 16px+ 볼드 폰트와 균형 잡힌 패딩으로 문제 본문과 자연스럽게 결합<br>5) **17-2 순서도 조립 캔버스 스크롤 및 작업 공간 확장**: 1~16번과 17-1은 스크롤 제로(Zero-Scroll)를 유지하고, 17-2에서만 세로 스크롤 허용 및 캔버스 최소 높이 640px+로 확장<br>6) **17-2 캔버스 잔여 음영/테두리 결함 제거**: 이중 테두리 및 그림자 제거로 라운드 모서리 완벽 밀착 마감<br>7) **토스/애플 스타일 마이크로 인터랙션**: 버튼/선지 스프링 터치 반응(`active:scale-96`) 및 문항 페이드인 트랜지션 적용<br>8) **75개 전체 단위 테스트 100% 통과** (3개 신규 테스트 추가) |
