...
최근 완료한 제품 변경은 성적표 멀티 시트 엑셀 개편 및 학생용 답안/정답 확인 모달 구축, 그리고 개별 학생 성적표 시트의 **A4 1장 무수정 맞춤 인쇄 및 좌우 2단 배치 최적화**입니다. 지필평가 16문항 전체의 학생 답안, 실제 정답, 채점 결과를 온전히 보여주면서도 세로 길이를 획기적으로 줄이기 위해 **좌우 2단 그리드(좌: Part 1 객관식 Q1~Q10, 우: Part 2 단답형 Q11~Q16 및 채점 안내 카드)** 구조로 개편했습니다. 전체 행 수를 정확히 23행으로 압축하고 `<pageSetUpPr fitToPage="1"/>` 설정을 적용하여 무수정 원클릭 인쇄 시 A4 1장에 완벽히 수용됩니다. 또한 셀 너비(A~F열: 14, 28, 14, 12, 26, 18)를 대폭 확장하고 라벨 셀에 `wrapText="1"`을 적용하여 '지필평가 소계', '종합 최종 점수', '상황/현재상태', '교사 종합 의견' 및 4대 기준 AI 상세 피드백의 문구 잘림 현상을 완벽히 해결했습니다. (단위 테스트 100/100 통과, tools/check.cjs 68개 스크립트 정적 검사 통과)
