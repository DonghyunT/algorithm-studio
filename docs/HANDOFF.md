...
|---|---|
| 현재 운영 브랜치 | main |
| 현재 작업 브랜치 | codex/cbt-ui-polish-round2 (CBT 2차 UI/UX 고도화: 지필 2단 분할·슬림 레일·캔버스 섬 밀봉) |
| 인수인계 전달 기준 | 최신 main(3829d56) 기반 CBT 2차 UI/UX 고도화 구현 및 단위 테스트 71개 통과(통과율 100%) |
| 최신 제품 코드 | CBT 지필 2단 분할(`cbt-question-grid`), 슬림 레일 트랜지션(56px), 17-2 캔버스 오버플로우 밀봉, `AssessmentAutoReviewQueue`, `formatNeisCSVRows`, 교사용 좌석 비우기/강제 제출/자동 마감 |
| 배포 기록 | Vercel Production 배포 대기 (main 병합 후 자동 배포 예정) |
| 현재 평가 UI | CBT 단일 문항 모드가 기본값 (`isCbtMode = true`), 우측 상단 '클래식 탭 모드' 토글 지원 |
| 이번 구현 완료 내용 | 1) **지필고사형 좌우 2열 분할 레이아웃**: 긴 문제(9번 등)에서도 보기가 아래로 밀리지 않고 스크롤 제로 및 고정 버튼 유지<br>2) **문항목록 사이드바 슬림 레일(Rail) 스무스 축소 트랜지션**: 접기(`<`) 클릭 시 56px 슬림 바로 부드럽게 축소, 현재 번호 및 응답비율 유지, 즉시 펼치기 지원<br>3) **순서도 조립(17-2) 캔버스 섬 완벽 밀봉**: 도트 그리드가 둥근 배경 밑으로 튀어나오는 현상 차단(`overflow: hidden !important`, `flex-1 min-h-0`), 1단계 요약 인라인 아코디언화<br>4) 71개 전체 단위 테스트 통과 (통과율 100%) |
