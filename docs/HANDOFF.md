...
|---|---|
| 현재 운영 브랜치 | main |
| 현재 작업 브랜치 | codex/cbt-assessment-layout-plan (KERIS 국가수준 CBT 컴퓨터 모니터 가독성 & 13대 피드백 고도화 완료) |
| 인수인계 전달 기준 | 최신 main 및 작업 브랜치 codex/cbt-assessment-layout-plan. 검토 후 main 병합 예정 |
| 이번 작업 시작 기준 | 84c1f68 (main) → 39f0b39 (디벗 최적화) → CBT 컴퓨터 모니터 가독성 및 13대 피드백 고도화 |
| 최신 제품 코드 | 1) 데스크탑 메인 카드 높이 및 하단 버튼 위치 완전 고정, 2) SVG 번호 뱃지 광학 정렬, 3) 13/15번 지문 단락 줄바꿈, 4) 17번 중복 버튼 삭제 및 좌우 카드 시작 높이 칼일치, 5) 좌측 OMR 17-1/17-2 분할, 6) 17-1 교과 표준 용어화, 7) 처방전 탈피 및 '나의 문제 해결 계획' 개편, 8) 시작/목표 상태 가로 2열 배치, 9) 40점 채점 기준 박스 좌측 하단 배치, 10) 사이드바 접기/펼치기 및 17-2 캔버스 자동 접힘, 11) 17-2 요약 바 규칙(조건) 표시, 12) 지문 내 독립 [조건 카드 상자] 테두리 시각화, 13) '제출하고 이동하기' ➔ 학생 안심 '다음 문항 →' 버튼 문구 개편 |
| 배포 기록 | c48e232 Firestore 규칙 게시, c73e8c4 Vercel API 반영 (운영 기준) |
| 현재 평가 UI | CBT 단일 문항 모드가 기본값 (`isCbtMode = true`), 우측 상단 '클래식 탭 모드' 토글 지원 |
| 이번 구현 완료 내용 | 선생님의 13대 현장 피드백 완벽 반영, `node tools/check.cjs` 구문 검사 및 `tests/*.test.cjs` 60개 단위 테스트 전원 통과 (통과율 100%) |
