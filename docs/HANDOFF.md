...
| 이번 문서 작업 시작 기준 | 7d24e50 — 이전 인수인계 main 통합 완료. 로컬 미커밋 변경 없음 확인 |
...
2. [V4 계획 8~11절](EVALUATION_V4_PLAN.md#8-다음-순서와-활성화-조건)을 먼저 읽습니다. 현재 사실 → 위험 → 권장 방향 → 결정 필요 → 완료 조건을 구분했습니다. **기반을 재구축하지 말고 회차 재현과 비공개 보관 설계부터 확인합니다.** [운영 설정](EVALUATION_V4_SETUP.md)은 현재 P0 설명이며 설정만으로 V4 준비가 끝나지 않습니다.
...
4. 회차별 문항·보기 순서·배정·채점 기준과 교사 확정 성적의 보존·복구 방식을 먼저 비교합니다. 보관 서비스·기간·키 교체·성적 정정은 미정입니다. 문항 검토 뒤 승인된 구현 범위에서 API/직접 DB 권한·저장 실패·과거 회차 검증을 진행하고 소규모 확인 후 확대합니다. 문서 승인이나 과거 main 통합 승인을 V4 활성화 승인으로 확대하지 않습니다.
